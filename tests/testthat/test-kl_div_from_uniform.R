...
test_that("example output", {
  spec <- c(0.2, 0.4, 0.3, 0.1) %>% .pc_smooth_spectrum()
  unif <- 1 / length(spec)
  d <- 0
...
test_that("invariance to the number of bins", {
  ...
  v2 <- rep(v1, each = 2)
  s1 <- .pc_smooth_spectrum(v1)
  s2 <- .pc_smooth_spectrum(v2)
  expect_equal(
...
test_that("Peak measure produces different patterns to KL measure", {
  organised_sub_peaks <- c(0, 0, 3, 0, 0, 5, 0, 0, 0, 0) %>% .pc_smooth_spectrum()
  disorganised_sub_peaks <- c(1, 1, 1, 0, 0, 5, 0, 0, 0, 0) %>% .pc_smooth_spectrum()
  expect_equal(
